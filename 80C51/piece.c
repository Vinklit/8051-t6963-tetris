...
void PIECE_initialize(Piece *piece, TetrominoType type, unsigned char x, unsigned char y, unsigned char orientation) {
	...
	piece->block.dta = tetromino[type].dta;
	piece->position.x = x;
	piece->position.y = y;
...
/**
 * Rend le caractère qui se trouve dans la pièce à la position
 * spécifiée.
 * @param piece La description de la pièce.
 * @param positionInPiece La position relative dans la pièce. La
 * position 0,0 correspond à l'angle supérieur droit.
 */
...
/**
 * Calcule la position absolue à l'écran qui correspond à la position relative
 * spécifiée sur la pièce.
 * @param piece La définition de la pièce, y compris son orientation et
 * sa position.
 * @param piecePosition Les coordonnées relatives à la pièce.
 * @param screenPosition La fonction dépose ici la position absolue à l'écran.
 */ 
void calculateScreenPosition(Piece *piece, Position *piecePosition, Position *screenPosition) {
	...
	switch(orientation) {
		...
		//             DEF
		case 0:
			screenPosition->x = piecePosition->x;
			screenPosition->y = piecePosition->y;
			break;
		...
		//             FC
		case 1:
			screenPosition->x = piece->block.height - piecePosition->y - 1;
			screenPosition->y = piecePosition->x;
			break;
		...
		//
		case 2:
			screenPosition->x = piece->block.width - piecePosition->x - 1;
			screenPosition->y = piece->block.height - piecePosition->y - 1;
			break;
		...
		case 3:
		default:
			screenPosition->x = piecePosition->y;
			screenPosition->y = piece->block.width - piecePosition->x - 1;
			break;
	}
...
/**
 * Indique si la pièce, dans sa rotation et position actuelle,
 ...
 * @param piece La description de la pièce.
 * @return 1 si la pièce peut être placée à cet endroit.
 */
...
void PIECE_place(Piece *piece) {
	unsigned char pieceContent;
	Position piecePosition, screenPosition;

	for (piecePosition.y = 0; piecePosition.y < piece->block.height; piecePosition.y++) {
		for (piecePosition.x = 0; piecePosition.x < piece->block.width; piecePosition.x++) {
			pieceContent = readPieceContent(piece, &piecePosition);
			if (pieceContent != EMPTY) {
				calculateScreenPosition(piece, &piecePosition, &screenPosition);
				T6963C_writeAt(screenPosition.x, screenPosition.y, pieceContent);
			}
		}
	}
}
