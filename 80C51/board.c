...
void BOARD_draw(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1) {
	...
	T6963C_dataWrite(address++, BORDER_A);
	address = T6963C_autoRepeat(address, BORDER_B, x1 - x0 - 1);
	T6963C_dataWrite(address++, BORDER_C);

	for (y = y0 + 1; y < y1; y++) {
		T6963C_writeAt(x0, y, BORDER_D);
		T6963C_writeAt(x1, y, BORDER_E);
	}

	address = T6963C_calculateAddress(x0, y1);
	T6963C_dataWrite(address++, BORDER_F);
	address = T6963C_autoRepeat(address, BORDER_G, x1 - x0 - 1);
	T6963C_dataWrite(address++, BORDER_H);
}
...
void BOARD_clear(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1) {
	...
	for (y = y0; y <= y1; y++) {
		address =  T6963C_calculateAddress(x0, y);
		T6963C_autoRepeat(address, EMPTY, x1 - x0 + 1);
	}
}
 ...
 * trouvent au dessus.
 * @param line La position de la ligne à effacer.
 */
